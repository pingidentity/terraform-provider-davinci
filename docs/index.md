...
The following assumes that the PingOne worker app has been provided the `DaVinci Admin` and `Identity Data Admin` roles scoped to the PingOne environment containing the Identity, and the `Organization Admin` role scoped to the organization.
