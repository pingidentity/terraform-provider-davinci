...
-> The legacy DaVinci provider relied on human user credentials and browser-based SSO. The PingOne Terraform provider uses PingOne worker applications, eliminating the dependency on human credentials. This is significantly more suitable for automation scenarios such as CI/CD pipelines or GitHub Actions workflows.
